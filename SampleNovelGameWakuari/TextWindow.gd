...
var end = false	#Tweenの終了状態を示すフラグ
...
func _process(delta):
	...
	if Input.is_action_just_pressed("click"):	#左クリックされたら
...
func show_text():
	if Global.lines < Global.mes.size():		#linesがmes配列のサイズを超えていないなら
		...
		#「$Text.get_total_character_count()/25」は1行の文字総数を25で割ることで表示速度を一定に保っている。
		$Text/Tween.interpolate_property($Text, "percent_visible", 0, 1, $Text.get_total_character_count()/25, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
